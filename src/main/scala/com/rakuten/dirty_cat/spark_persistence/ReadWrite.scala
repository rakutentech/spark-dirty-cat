 ...
 */
package com.rakuten.dirty_cat.persistence

import com.rakuten.dirty_cat.utils.Utils

import org.apache.hadoop.fs.Path
import org.apache.spark.SparkContext
import org.apache.spark.ml.param.{ParamPair, Params}
import org.apache.spark.ml.util.{MLReader, MLWriter}
import org.json4s.JsonAST.{JObject, JValue, JArray, JField}
...
private[dirty_cat] object DefaultParamsWriter {
  ...
  def getMetadataToSave(
    ...
    val jsonParams = (paramMap.getOrElse(render(params.map{ case ParamPair(p, v) => p.name -> parse(p.jsonEncode(v))}.toList)))
...
private[dirty_cat] object DefaultParamsReader {
